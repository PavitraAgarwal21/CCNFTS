...
mod ChessLogic {
    ...
    fn isLegalMove(_board: u256, _move: u256) -> bool {
     ...
     fn searchRay(
     _board: u256, _fromIndex: u256, _toIndex: u256, _directionVector: u256
    ) -> bool {
...
impl IChessLogicImp of IChessLogicTrait {
            ...
            if (piece == 1) {
                ...
                while (move != 0) {
                    if (isValid(_board, adjustedIndex + (move & 0xFF))) {
                        self.appendTo(ref movesArray, adjustedIndex, adjustedIndex + (move & 0xFF));
                    }
    ...
    // here append to index is changed to and tested more  
     fn appendTo(self : @ContractState , ref _moveArray: MoveArray, _fromMoveIndex: u256, _toMoveIndex: u256) -> bool {
        ...
        fn _play_move_chess(ref self : ContractState ,  _board: u256, _move: u256, _depth: u256 , ai : bool )  {
